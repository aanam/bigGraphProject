var d3Graph = (function () {
    ...
    function visualize(elementOrId, w, h, data) {
        ...
        console.log('w',w,'h',h,'k',k);

        var fill = d3.scale.category10();
        var select = ( typeof elementOrId === 'string' ) ? '#' + elementOrId : elementOrId;
        var vis = d3.select(select).append('svg').attr('class', 'd3-graph')
            .attr('height', h)
            .attr('width', w)
        ...
        // end-of-line arrow
        vis.append('svg:defs').selectAll('marker').data([{id:'end-marker', color:"gray"}, {id:'end-marker-selected', color:"red"} ]) // link types if needed
        ...
        var link = vis.selectAll('line.link').data(data.links).enter().append('svg:line').attr('class', 'd3-link')
            ...
            .style('stroke',function (d) { return d['selected'] ? 'red' : 'gray'; })
    ...
    function render(id, style, w, h, graph) {
        // d3.json(url, function (data) {
        //     visualize(id, w, h, graph);
        // });
        if (style == "plain") {
            visualize(id, w, h, graph);
        }
